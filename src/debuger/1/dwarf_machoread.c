    ...
    Dwarf_Small **return_data, int *error)
{
    ...
    if (0 < section_index &&
        ...
        res = RRMOA(macho->mo_fd,
            sp->loaded_data, (int)sp->offset,
            (int)sp->size, (int)macho->mo_filesize, error);
        if (res != DW_DLV_OK) {
            free(sp->loaded_data);
            sp->loaded_data = 0;
            return res;
    ...
    int *errcode)
{
    ...
    res = RRMOA(mfp->mo_fd, &sc, (int)mmp->offset_this_command,
        sizeof(sc), (int)filesize, errcode);
    if (res != DW_DLV_OK) {
        return res;
    }
    ...
    Dwarf_Unsigned segi, int *errcode)
{
    ...
    for (; seci < secalloc; ++seci,++secs,curoff += shdrlen ) {
        ...
        res = RRMOA(mfp->mo_fd, &mosec, (int)curoff, sizeof(mosec),
            (int)mfp->mo_filesize, errcode);
        if (res != DW_DLV_OK) {
            return res;
        }
    ...
    dwarf_macho_object_access_internals_t *mfp,int *errcode)
{
    ...
    for ( ; cmdi < mfp->mo_header.ncmds; ++cmdi,++mcp ) {
        res = RRMOA(mfp->mo_fd, &mc, (int)curoff, sizeof(mc),
            (int)mfp->mo_filesize, errcode);
        if (res != DW_DLV_OK) {
            return res;
        }
