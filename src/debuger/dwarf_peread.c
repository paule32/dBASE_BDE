    ...
    Dwarf_Unsigned offset, int*errcode)
{
    ...
    res =  _dwarf_object_read_random(pep->pe_fd,
        (char *)&hdr,
        (int)offset, sizeof(IMAGE_OPTIONAL_HEADER32_dw),
        (int)pep->pe_filesize,
        errcode);
    if (res != DW_DLV_OK) {
        return res;
    ...
    Dwarf_Unsigned offset, int*errcode )
{
    ...
    res =  _dwarf_object_read_random(pep->pe_fd,
        (char *)&hdr,
        (int)offset, sizeof(IMAGE_OPTIONAL_HEADER64_dw),
        (int)pep->pe_filesize,
        errcode);
    if (res != DW_DLV_OK) {
        return res;
    ...
    Dwarf_Small **return_data, int *error)
{
    ...
    if (0 < section_index &&
        ...
        res = _dwarf_object_read_random(pep->pe_fd,
            (char *)sp->loaded_data,
            (int)sp->PointerToRawData, (size_t)read_length,
            (int)pep->pe_filesize,
            error);
        if (res != DW_DLV_OK) {
            free(sp->loaded_data);
    ...
    dwarf_pe_object_access_internals_t *pep,int *errcode)
{
    ...
    res = _dwarf_object_read_random(pep->pe_fd,(char *)&dhinmem,
        0, sizeof(dhinmem),(int)pep->pe_filesize, errcode);
    if (res != DW_DLV_OK) {
        return res;
    }
    ...
    res =  _dwarf_object_read_random(pep->pe_fd,
        (char *)&nt_sig_array[0],
        (int)nt_address, sizeof(nt_sig_array),
        (int)pep->pe_filesize,errcode);
    if (res != DW_DLV_OK) {
        return res;
    }
    ...
    res = _dwarf_object_read_random(pep->pe_fd,(char *)&ifh,
        (int)pep->pe_nt_header_offset, sizeof(ifh),
        (int)pep->pe_filesize,errcode);
    if (res != DW_DLV_OK) {
        return res;
    }
    ...
    if (pep->pe_string_table_offset) {
        ...
        res =  _dwarf_object_read_random(pep->pe_fd,
            (char *)size_field, (int)pep->pe_string_table_offset,
            sizeof(size_field),
            (int)pep->pe_filesize,errcode);
        if (res != DW_DLV_OK) {
            return res;
        ...
        res = _dwarf_object_read_random(pep->pe_fd,
            ...
            (int)pep->pe_string_table_offset,
